var myapp = angular.module("myapp", []).controller(
		...
		function($scope, $http) {
			...
			$scope.getProductsList = function() {
				$http.get('http://localhost:8008/springweb/getProductsList')
						.success(function(data) {
							$scope.products = data;
						})
			}
			$scope.addToCart = function(isbn) {
				$http.put('http://localhost:8008/springweb/cart/add/' + isbn)
						.success(function() {
			...
			$scope.refreshCart = function() {
				$http.get(
						'http://localhost:8008/springweb/cart/getCart/'
			...
			$scope.removeFromCart = function(cartItemId) {
				$http.put(
						'http://localhost:8008/springweb/cart/removecartitem/'
			...
			$scope.clearCart = function() {
				$http.put(
						'http://localhost:8008/springweb/cart/removeAllItems/'
